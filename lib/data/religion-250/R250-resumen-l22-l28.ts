import type { Lesson } from "@/lib/types"

export const leccionesResumidasR250_22_28: Lesson[] = [
  {
    id: "r250-l22",
    title: "Lección 22: Participar con Jesucristo en la Restauración continua",
    type: "Religión 250",
    description: "",
    unitTitle: "El ministerio posterrenal de Jesucristo",
    unitNumber: 5,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-22-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "El 28 de octubre de 2018, tras la dedicación del Templo de Concepción, Chile, el presidente Nelson habló con periodistas sobre lo que podía esperar el mundo de la Iglesia en los años venideros. Dijo: «Somos testigos de un proceso de restauración. Si creen que la Iglesia ha sido completamente restaurada, sólo están viendo el comienzo. Queda mucho por venir. Esperen hasta el próximo año y luego el año que viene. Tomen sus vitaminas; descansen bien. Va a ser emocionante». Lo que siguió confirmó esas palabras: cambios en la estructura del sacerdocio, nuevo programa de ministración, estudio centrado en el hogar, construcción de templos a ritmo sin precedentes, y la «Proclamación de la Restauración» en el bicentenario de la Primera Visión.\n\nCuando pensamos en la Restauración, el instinto es ubicarla en el pasado: la Arboleda Sagrada, el Libro de Mormón, José Smith, 1830. Eso es cierto, pero incompleto. El Artículo de Fe 9 lo dice claramente: creemos «todo lo que Dios ha revelado, todo lo que actualmente revela» y los «muchos grandes e importantes asuntos que aún revelará». La Restauración no fue un evento cerrado — fue un comienzo. El presidente Uchtdorf lo describió con la imagen del cuento de Rip Van Winkle: un hombre que se duerme y cuando despierta, el mundo ya cambió sin él. La pregunta que lanzó fue directa: «¿Están durmiendo durante la Restauración?».\n\nEso tiene implicaciones personales muy concretas para un joven universitario de hoy. La profecía de Daniel 2:44 describe el reino de Dios como una piedra que «llenará toda la tierra» — y ese crecimiento está ocurriendo ahora, en tu generación. El presidente Nelson describió tres formas de participar activamente en esa obra: ser personas de convenios firmes, reunir a Israel de este lado del velo mediante la obra misional, y reunirlo del otro lado mediante la obra del templo. No son actividades de ancianos o de misioneros de tiempo completo: son la vocación de toda alma bautizada que entiende en qué momento histórico vive.",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "Daniel 2:44",
            texto:
              "Y en los días de estos reyes, el Dios del cielo levantará un reino que no será jamás destruido, ni será el reino dejado a otro pueblo; desmenuzará y consumirá a todos estos reinos, pero él permanecerá para siempre.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/ot/dan/2?lang=spa&id=p44#p44",
          },
          {
            referencia: "Doctrina y Convenios 65:2",
            texto:
              "El reino del cielo es como el grano de mostaza, que un hombre tomó y sembró en su campo; el cual a la verdad es la más pequeña de todas las semillas; pero cuando ha crecido, es la mayor de las hortalizas, y se hace árbol, de tal manera que las aves del cielo vienen y hacen nidos en sus ramas.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/65?lang=spa&id=p2#p2",
          },
          {
            referencia: "Artículos de Fe 1:9",
            texto:
              "Creemos todo lo que Dios ha revelado, todo lo que actualmente revela, y creemos que Él revelará aún muchos grandes e importantes asuntos pertenecientes al Reino de Dios.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/pgp/a-of-f/1?lang=spa&id=p9#p9",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "Presidente Dieter F. Uchtdorf",
        fuente:
          "«¿Están durmiendo durante la Restauración?», conferencia general (sesión de sacerdocio), abril de 2014.",
        texto:
          "A veces consideramos la Restauración del Evangelio como algo que está completo, que ya dejamos atrás: José Smith tradujo el Libro de Mormón, recibió las llaves del sacerdocio, se organizó la Iglesia. En realidad, la Restauración es un proceso en pleno desarrollo; la estamos viviendo ahora mismo. Cuando nuestro tiempo en la vida terrenal se haya terminado, ¿qué experiencias podremos compartir sobre nuestra contribución a este período significativo? Hay demasiado en juego para hacer las cosas a medias en esta obra sagrada.",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2014/04/are-you-sleeping-through-the-restoration?lang=spa&id=p9-9#p9",
        // Esta cita combina fragmentos reales de los párrafos 9, 11 y 59 del discurso (con elipsis); el ancla apunta solo al primer fragmento (p9) porque un rango p9-59 resaltaría casi todo el discurso.
      },
      {
        tipo: "enseñanza",
        autor: "Presidente Russell M. Nelson",
        fuente:
          "«La Restauración de la plenitud del Evangelio de Jesucristo: Una proclamación para el mundo en el bicentenario», 5 de abril de 2020.",
        texto:
          "Han transcurrido doscientos años desde que Dios el Padre y Su Hijo Amado, Jesucristo, dieron inicio a esta Restauración. Millones de personas en todo el mundo han aceptado el conocimiento de estos acontecimientos que fueron predichos. Con alegría declaramos que la Restauración prometida avanza por medio de la revelación continua.",
        link: "https://www.churchofjesuschrist.org/study/scriptures/the-restoration-of-the-fulness-of-the-gospel-of-jesus-christ?lang=spa&id=p7-8#p7",
      },
      {
        tipo: "conclusion",
        contenido:
          "Daniel vio un reino que empezaría pequeño como una piedra y terminaría llenando la tierra entera. DyC 65:2 describe ese mismo reino como el grano de mostaza que crece hasta hacerse árbol. Ambas imágenes tienen una cosa en común: el crecimiento es activo, imparable y progresivo. Vos sos parte de ese crecimiento ahora. Cada acto de fidelidad, cada testimonio compartido, cada nombre llevado al templo, cada día vivido en convenio es un ladrillo en ese edificio. La Restauración no es algo que tus abuelos vivieron — es algo que vos y tu generación están viviendo. El presidente Uchtdorf lanzó la pregunta. La respuesta depende de vos: ¿estás despierto?",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "El presidente Uchtdorf pregunta si «estamos durmiendo durante la Restauración». ¿De qué manera concreta estás participando activamente en ella — no de manera pasiva o por defecto, sino con intención?",
          "El Artículo de Fe 9 dice que Dios «actualmente revela» y «aún revelará». ¿Cómo te preparás para recibir revelación nueva, personal e institucional, en vez de solo preservar lo que ya conocés?",
          "Daniel 2:44 dice que el reino llenará «toda la tierra». ¿Qué papel específico podés jugar en ese crecimiento desde tu posición actual — como estudiante, en tu barrio, en tu familia, en tu facultad?",
          "El presidente Nelson dijo «tomen vitaminas, descansen — va a ser emocionante». ¿Cómo cambia tu actitud hacia la Iglesia y sus demandas si la ves como participante en el evento más importante de la historia, en vez de como obligación?",
          "¿Cuál de las tres formas de participación en la Restauración — convenios firmes, obra misional de este lado del velo, obra del templo — es la más descuidada en tu vida ahora mismo? ¿Por qué?",
          "DyC 65:2 (grano de mostaza) y Daniel 2:44 (piedra que llena la tierra) describen el mismo reino con imágenes de crecimiento orgánico e imparable. Artículos de Fe 1:9 añade que ese crecimiento incluye revelación continua. ¿Cuál es el principio que une los tres, y qué responsabilidad le asigna al individuo que lo entiende?",
        ],
      },
    ],
  },
  {
    id: "r250-l23",
    title: "Lección 23: Vivir con esperanza conforme nos preparamos para la segunda venida de Jesucristo",
    type: "Religión 250",
    description: "",
    unitTitle: "El ministerio posterrenal de Jesucristo",
    unitNumber: 5,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-23-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "Si mencionás la Segunda Venida de Cristo en un grupo de jóvenes universitarios, las reacciones suelen ir en dos direcciones. La primera es la ansiedad — imágenes apocalípticas, guerras, calamidades, la sensación de que el mundo se está desmoronando. La segunda es la indiferencia — «eso está muy lejos, ya veremos». Ninguna de las dos es la respuesta del Evangelio.\n\n«El Cristo Viviente» incluye, hacia el final, una promesa que los Apóstoles firman: «Testificamos que algún día Él regresará a la tierra.» No es una amenaza — es la declaración de que todo lo que hemos esperado tiene un cumplimiento glorioso garantizado. DyC 101:36 lo dice con directriz explícita: «No temáis, porque el reino os pertenece.» El tiempo presente importa: el reino ya pertenece a quienes están en el camino del convenio. La Segunda Venida no inaugura ese reino — lo completa.\n\nEl desafío real no es calcular fechas ni acumular provisiones físicas — el Salvador mismo dijo que «nadie sabe el día ni la hora» (Mateo 24:36). El desafío es vivir de manera que cuando Él llegue, sea la continuación natural de la vida que ya estamos llevando. Lucas 12:40 dice «estad listos», no «estad aterrorizados». La preparación que el Señor pide es de carácter, de convenios y de servicio cotidiano — exactamente lo que el curso entero de Religión 250 ha enseñado. En ese sentido, esta lección es menos sobre el futuro que sobre el presente: ¿cómo vivo hoy a la luz de lo que sé que viene?",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "Doctrina y Convenios 45:16",
            texto:
              "Y también de las señales que aparecerán en los últimos días ante mi venida, he aquí que hablaré en nombre de Cristo; por tanto, escuchad mis palabras.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/45?lang=spa&id=p16#p16",
          },
          {
            referencia: "Doctrina y Convenios 101:36",
            texto:
              "Por tanto, no temáis, pequeño rebaño; haced el bien; que es la voluntad de vuestro Padre daros el reino.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/101?lang=spa&id=p36#p36",
          },
          {
            referencia: "Lucas 12:40",
            texto:
              "Vosotros también, estad preparados, porque a la hora que no penséis, el Hijo del Hombre vendrá.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/luke/12?lang=spa&id=p40#p40",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "Presidente Russell M. Nelson",
        fuente:
          "«El Señor Jesucristo vendrá de nuevo», conferencia general, octubre de 2024.",
        texto:
          "Mis queridos hermanos y hermanas, ¿ven lo que está sucediendo ante nuestros ojos? ¡Ruego que no pasemos por alto la majestuosidad de este momento! El Señor ciertamente está apresurando Su obra. Las bendiciones del templo ayudan a recoger a Israel en ambos lados del velo. ¡Y esas bendiciones también ayudan a preparar a un pueblo que ayudará a preparar al mundo para la Segunda Venida del Señor!",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2024/10/57nelson?lang=spa&id=p_leSJJ-p_hbgf4#p_leSJJ",
        // Este discurso (2024) usa un esquema de id de párrafo nuevo (hash alfanumérico, no numérico secuencial); el ancla de párrafo es un mejor esfuerzo y puede no resaltar el rango exacto, aunque el salto de scroll (#p_leSJJ) sí funciona.
      },
      {
        tipo: "enseñanza",
        autor: "«El Cristo Viviente: El Testimonio de los Apóstoles»",
        fuente:
          "Primera Presidencia y el Cuórum de los Doce Apóstoles, 1 de enero de 2000.",
        texto:
          "Testificamos que algún día Él regresará a la tierra... Él regirá como Rey de reyes y reinará como Señor de señores, y toda rodilla se doblará, y toda lengua hablará en adoración ante Él... Él es la luz, la vida y la esperanza del mundo. Su camino es el sendero que lleva a la felicidad en esta vida y a la vida eterna en el mundo venidero.",
        link: "https://www.churchofjesuschrist.org/study/scriptures/the-living-christ-the-testimony-of-the-apostles?lang=spa&id=p12-13#p12",
      },
      {
        tipo: "conclusion",
        contenido:
          "La Segunda Venida no es para temerla ni para ignorarla — es para vivirla como horizonte. Un corredor que sabe dónde está la meta no corre con miedo sino con dirección. Las señales de los últimos días que el Salvador describió en DyC 45 son indicadores de que Su promesa se está cumpliendo, no amenazas contra los fieles. La mejor preparación para el regreso del Salvador es exactamente lo que el Evangelio siempre ha pedido: hacer y guardar convenios, ministrar a quienes nos rodean, perseverar con fe cada día. Si estás haciendo eso, la Segunda Venida no debería producirte ansiedad — debería producirte esperanza.",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "Cuando pensás en la Segunda Venida de Cristo, ¿qué sentimiento predomina en vos — esperanza, indiferencia, o ansiedad? ¿Qué dice eso sobre cómo estás viviendo el Evangelio ahora mismo?",
          "DyC 101:36 dice «no temáis» y Lucas 12:40 dice «estad preparados». ¿Cómo se relacionan esas dos instrucciones? ¿Es posible estar preparado sin temer, y qué diferencia a los dos estados?",
          "El presidente Nelson dijo que los templos «ayudan a preparar un pueblo para la Segunda Venida». ¿Por qué la preparación para ese evento pasa por el templo y no por acumular conocimiento profético o provisiones físicas?",
          "Si alguien te dijera «me estresa pensar en la Segunda Venida porque siento que no estoy listo», ¿qué principio de esta lección — y del curso completo — le ofrecerías como respuesta?",
          "«El Cristo Viviente» testifica que Cristo «regirá como Rey de reyes y reinará como Señor de señores» y que Su camino «lleva a la felicidad en esta vida». ¿Cómo conectan esas dos afirmaciones — el futuro eterno y la felicidad presente?",
          "DyC 45:16 habla de «señales» antes de la venida. DyC 101:36 dice «no temáis». «El Cristo Viviente» testifica que Él «regresará a la tierra». ¿Cuál es el principio que une los tres pasajes y que define cómo un discípulo debe relacionarse con los eventos de los últimos días?",
        ],
      },
    ],
  },
  {
    id: "r250-l24",
    title: "Lección 24: Confiar en Jesucristo como nuestro Juez",
    type: "Religión 250",
    description: "",
    unitTitle: "El ministerio posterrenal de Jesucristo",
    unitNumber: 5,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-24-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "El Juicio Final es uno de los conceptos que más ansiedad genera en personas de fe — incluso en quienes han intentado vivir bien. Hay una imagen mental de estar solo ante un tribunal mientras toda la vida se proyecta en pantalla grande. Pero la doctrina restaurada transforma completamente esa imagen con un dato que lo cambia todo: el Juez ante quien compareceremos es el mismo Salvador que fue a Getsemaní por nosotros.\n\nDyC 45:3–5 describe a Cristo ante el Padre no como fiscal sino como Abogado: «Escucha a aquel que es el abogado ante el Padre... diciendo: Padre, contempla el sufrimiento y la muerte de aquel que no pecó... Por tanto, Padre, no entres en juicio con tu siervo.» El mismo que pagó la deuda es quien intercede. Eso no elimina la responsabilidad personal — DyC 137:9 dice que el Señor juzgará «según sus obras y según el deseo de sus corazones» — pero sí cambia radicalmente el tono del encuentro.\n\nEl presidente Oaks añadió una perspectiva que transforma la manera de entender el propósito del Evangelio: «El Juicio Final no es simplemente una evaluación de la suma total de las obras buenas y malas. Es un reconocimiento del efecto final que tienen nuestros hechos y pensamientos — lo que hemos llegado a ser.» Eso significa que el objetivo del Evangelio no es acumular puntos de mérito sino transformar el carácter. Cada convenio, cada acto de servicio, cada momento de oración sincera no es un depósito en una cuenta celestial — es práctica deliberada de convertirse en la persona que el Padre quiere que seas.",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "Doctrina y Convenios 45:3–5",
            texto:
              "Escucha a aquel que es el abogado ante el Padre, que está intercediendo tu causa ante él, diciendo: Padre, contempla el sufrimiento y la muerte de aquel que no pecó, en el cual tú te complaciste; contempla la sangre del Hijo tuyo, que fue derramada... Por tanto, Padre, no entres en juicio con tu siervo.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/45?lang=spa&id=p3-p5#p3",
          },
          {
            referencia: "Doctrina y Convenios 137:9",
            texto:
              "Porque yo, el Señor, juzgaré a todos los hombres según sus obras, según el deseo de sus corazones.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/137?lang=spa&id=p9#p9",
          },
          {
            referencia: "3 Nefi 27:27",
            texto:
              "Por tanto, ¿qué clase de hombres debéis ser? En verdad os digo que así como yo soy.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/bofm/3-ne/27?lang=spa&id=p27#p27",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "Presidente Dallin H. Oaks",
        fuente:
          "«El desafío de lo que debemos llegar a ser», Liahona, enero de 2001, pág. 40.",
        texto:
          "El Juicio Final no es simplemente una evaluación de la suma total de las obras buenas y malas, o sea, lo que hemos hecho. Es un reconocimiento del efecto final que tienen nuestros hechos y pensamientos, o sea, lo que hemos llegado a ser. Los mandamientos, las ordenanzas y los convenios del Evangelio no son una lista de depósitos que tenemos que hacer en alguna cuenta celestial. El evangelio de Jesucristo es un plan que nos muestra cómo llegar a ser lo que nuestro Padre Celestial desea que lleguemos a ser.",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2000/10/the-challenge-to-become?lang=spa&id=p6-6#p6",
      },
      {
        tipo: "conclusion",
        contenido:
          "Hay una convergencia poderosa entre las lecciones finales de este curso: ministrar como el Salvador (L13), desarrollar Sus atributos (L26), ser Sus testigos (L28) — todo apunta al mismo destino del Juicio. No porque Dios quiera sorprendernos, sino porque quiere que seamos como Él. La pregunta que el presidente Oaks plantea no es «¿hiciste suficiente?» sino «¿en quién te estás convirtiendo?». Esa es la pregunta más honesta que cualquier estudiante de Instituto puede hacerse al final de un semestre de estudio sobre Jesucristo.",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "¿Cómo cambia tu comprensión del Juicio Final saber que el Juez es el mismo que fue a Getsemaní por vos y que hoy intercede como tu Abogado ante el Padre?",
          "DyC 137:9 dice que el Señor juzgará «según el deseo de sus corazones». ¿Qué dice eso sobre la importancia de la motivación interna — no solo la conducta externa — en el Evangelio?",
          "El presidente Oaks dice que el Evangelio no es una «lista de depósitos» sino un plan para llegar a ser. ¿Cómo cambia esa perspectiva la manera en que te acercás a los mandamientos y las ordenanzas?",
          "3 Nefi 27:27 dice «¿qué clase de hombres debéis ser? Así como yo soy.» ¿Cuál es el atributo de Cristo que más necesitás desarrollar para que esa respuesta sea real en tu vida?",
          "Alguien te dice: «El Juicio me da mucho miedo — siento que nunca voy a ser suficientemente bueno». Según DyC 45:3–5 y la enseñanza de Oaks, ¿qué le dirías para darle esperanza sin minimizar la responsabilidad personal?",
          "DyC 45:3–5 muestra a Cristo como Abogado. DyC 137:9 dice que juzgará obras y deseos. 3 Nefi 27:27 dice que el estándar es ser «así como yo soy». ¿Cómo se complementan los tres pasajes para describir un Juicio que es a la vez justo y misericordioso?",
        ],
      },
    ],
  },
  {
    id: "r250-l25",
    title: "Lección 25: Esforzarse por ser un discípulo fiel de Jesucristo",
    type: "Religión 250",
    description: "",
    unitTitle: "Llegar a ser discípulos de Jesucristo",
    unitNumber: 6,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-25-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "Una mañana en el Mar de Galilea, Jesús vio a dos pescadores reparando sus redes. Se acercó, los miró, y dijo dos palabras: «Sígueme». Simón Pedro y Andrés dejaron las redes y lo siguieron. Esa escena parece simple desde afuera, pero encierra algo radical: dejaron una identidad conocida — somos pescadores, esto es lo que hacemos — por una invitación sin garantías de destino preciso. El discipulado comenzó con una respuesta a una persona, no con la adhesión a un sistema de creencias.\n\nLucas 9:23 da la versión que el Salvador mismo ofrece del costo: «Si alguno quiere venir en pos de mí, niéguese a sí mismo, tome su cruz cada día, y sígame.» Tres verbos en secuencia: negarse, tomar, seguir. El primero es el más difícil — negarse a sí mismo no significa volverse una persona sin deseos ni personalidad, sino no hacer del yo la instancia última de decisión. El segundo habla de una cruz, y habla de tomarla «cada día» — no en un momento heroico sino en la acumulación de días ordinarios. El tercero, seguir, implica movimiento y dirección: hay alguien a quien seguir, no solo principios que cumplir.\n\nJuan 8:31–32 añade la dimensión de la permanencia: «Si vosotros permaneciereis en mi palabra, seréis verdaderamente mis discípulos; y conoceréis la verdad, y la verdad os hará libres.» La palabra clave es «permaneciereis» — no quien comenzó con entusiasmo sino quien se quedó. En la universidad, el discipulado se prueba menos en los grandes momentos de decisión que en la consistencia de los días comunes: la honestidad cuando nadie mira, la compasión cuando estás cansado, la fe cuando los argumentos en contra parecen fuertes.",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "Lucas 9:23",
            texto:
              "Y decía a todos: Si alguno quiere venir en pos de mí, niéguese a sí mismo, tome su cruz cada día, y sígame.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/luke/9?lang=spa&id=p23#p23",
          },
          {
            referencia: "Juan 8:31–32",
            texto:
              "Dijo entonces Jesús a los judíos que habían creído en él: Si vosotros permaneciereis en mi palabra, seréis verdaderamente mis discípulos; y conoceréis la verdad, y la verdad os hará libres.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/john/8?lang=spa&id=p31-p32#p31",
          },
          {
            referencia: "Marcos 1:17–18",
            texto:
              "Y les dijo Jesús: Venid en pos de mí, y haré que seáis pescadores de hombres. Y dejando luego sus redes, le siguieron.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/mark/1?lang=spa&id=p17-p18#p17",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "Presidente Dieter F. Uchtdorf",
        fuente:
          "«¡Funciona de maravilla!», conferencia general, octubre de 2015.",
        texto:
          "Escuchar la palabra de Dios con verdadera intención nos lleva a creer en Dios y a confiar en Sus promesas. Debido a nuestro amor por Dios, deseamos seguirlo a Él y actuar en armonía con Su palabra... Dios los aceptará como son en este preciso momento y empezará a ayudarlos.",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2015/10/it-works-wonderfully?lang=spa&id=p37-48#p37",
      },
      {
        tipo: "conclusion",
        contenido:
          "El discipulado tiene un costo real — Jesús no prometió comodidad a quienes lo seguían. Pero tiene recompensas que lo exceden por mucho: la libertad que viene de conocer la verdad (Juan 8:32), la identidad clara de saber a quién pertenecemos, la paz profunda que proviene no de la ausencia de dificultades sino de la presencia de Su Espíritu. «Estad listos» y «sígueme» son la misma invitación: vivir ahora de manera que el discipulado sea ya una realidad en curso, no un proyecto futuro.",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "Lucas 9:23 dice «tome su cruz cada día». ¿Cuál es tu «cruz» específica en este momento de tu vida universitaria — aquello que el discipulado te pide que enfrentes todos los días?",
          "Pedro y Andrés «dejaron sus redes» — dejaron una identidad conocida para seguir a Cristo. ¿Hay alguna identidad, hábito o ambición que el discipulado te está pidiendo que dejes o reorienten en este momento?",
          "Juan 8:31 dice que el discipulado verdadero se prueba en «permanecer». ¿En qué aspectos de tu vida espiritual — oración, estudio, convenios, honestidad — te cuesta más permanecer cuando no hay presión social ni entusiasmo inmediato?",
          "El presidente Uchtdorf dice que, debido a nuestro amor por Dios, «deseamos seguirlo a Él y actuar en armonía con Su palabra». ¿Cómo se diferencia eso de simplemente cumplir reglas o asistir a reuniones?",
          "Si alguien observara tu semana completa sin conocerte — tu universidad, tus conversaciones, tus decisiones cuando nadie te ve — ¿qué evidencia encontraría de que sos discípulo de Jesucristo? ¿Qué evidencia faltaría?",
          "Lucas 9:23 dice «niéguese, tome, sígame». Juan 8:31 dice «permanecereis». Marcos 1:18 dice «dejando sus redes, le siguieron». ¿Cuál es el patrón que une los tres pasajes sobre el costo y el proceso del discipulado?",
        ],
      },
    ],
  },
  {
    id: "r250-l26",
    title: "Lección 26: Llegar a ser más semejantes a Jesucristo",
    type: "Religión 250",
    description: "",
    unitTitle: "Llegar a ser discípulos de Jesucristo",
    unitNumber: 6,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-26-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "Al final del Sermón del Monte en América, después de haber enseñado al pueblo nefita todo lo que enseñó en Palestina y más, Jesús hizo la pregunta más ambiciosa que se puede hacer a un ser humano: «¿Qué clase de hombres debéis ser? En verdad os digo que así como yo soy» (3 Nefi 27:27). No como un buen maestro. No como una persona moralmente correcta. Como Él es — el Hijo de Dios, lleno de gracia y verdad, con caridad perfecta, mansedumbre, paciencia, fe absoluta.\n\nEsa meta podría paralizarnos de humildad si no fuera por lo que el Evangelio enseña sobre cómo funciona el proceso. Moroni 7:47–48 describe la caridad — «el amor puro de Cristo» — como algo que no se fabrica con esfuerzo de voluntad sino que se pide: «Orad al Padre con toda la energía del corazón, para que seáis llenos de ese amor.» Es una invitación a la cooperación entre nuestro esfuerzo y la gracia. Nosotros ponemos la disposición y la práctica deliberada; Cristo añade lo que no podemos generar solos.\n\nLa lista de atributos de Cristo en la sección 4 de DyC — fe, esperanza, caridad, amor, ojo puesto en la gloria de Dios, virtud, conocimiento, templanza, paciencia, fraternidad, piedad — no es una descripción de lo que debés ser mañana. Es un horizonte. Y el avance hacia ese horizonte ocurre de la misma manera que el grano de mostaza creció: gradualmente, con perseverancia, bajo condiciones que a veces parecen adversas. Cada decisión difícil que tomás de manera congruente con esos atributos es práctica real. Cada vez que elegís la paciencia cuando la impaciencia sería más fácil, estás literalmente ejercitando un atributo divino.",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "3 Nefi 27:27",
            texto:
              "Por tanto, ¿qué clase de hombres debéis ser? En verdad os digo que así como yo soy.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/bofm/3-ne/27?lang=spa&id=p27#p27",
          },
          {
            referencia: "Moroni 7:45, 47–48",
            texto:
              "Y la caridad sufre con paciencia, y es benigna, y no tiene envidia, no se jacta de sí misma, no se ensoberbece... Mas la caridad es el amor puro de Cristo, y perdura para siempre; y al que se hallare que la tiene en el último día, le irá bien. Por tanto, amados hermanos, orad al Padre con toda la energía del corazón, para que seáis llenos de ese amor.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/bofm/moro/7?lang=spa&id=p45#p45",
          },
          {
            referencia: "Doctrina y Convenios 4:5–6",
            texto:
              "Y la fe, la esperanza, la caridad y el amor, con el ojo puesto en la gloria de Dios, califican a los hombres para la obra. Acordaos de la fe, la virtud, el conocimiento, la templanza, la paciencia, la fraternidad, la piedad, la caridad, la humildad, el celo.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/4?lang=spa&id=p5-p6#p5",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "Presidente Russell M. Nelson",
        fuente:
          "«Podemos actuar mejor y ser mejores», conferencia general (sesión de sacerdocio), abril de 2019.",
        texto:
          "Al escoger arrepentirnos, ¡escogemos cambiar! Permitimos que el Salvador nos transforme en la mejor versión de nosotros. Escogemos crecer espiritualmente y recibir gozo; el gozo de la redención en Él. Al escoger arrepentirnos, escogemos llegar a ser más semejantes a Jesucristo.",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2019/04/36nelson?lang=spa&id=p8-8#p8",
      },
      {
        tipo: "conclusion",
        contenido:
          "3 Nefi 27:27 no es una exigencia que aplasta — es una invitación que orienta. La meta «sé como yo soy» no produce culpa en quien la entiende bien: produce dirección. Cada atributo de Cristo que practicás hoy — la paciencia en el tráfico, la honestidad en el examen, la compasión con el compañero difícil — es un avance real hacia ese destino. El presidente Nelson lo resumió con una idea simple: al escoger arrepentirnos, escogemos llegar a ser más semejantes a Jesucristo. No perfección inmediata. Progreso intencional, decisión a decisión. Eso está al alcance de cualquiera.",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "3 Nefi 27:27 dice «sé como yo soy». ¿Te parece esa meta alcanzable, imposible, o algo intermedio? ¿Por qué, y cómo cambia tu respuesta la doctrina de la gracia habilitadora?",
          "Moroni 7:48 dice que la caridad no se genera sola — hay que orar «con toda la energía del corazón» para ser llenado de ella. ¿Qué dice eso sobre la relación entre el esfuerzo propio y el don divino en el desarrollo del carácter?",
          "DyC 4:5–6 lista atributos como fe, virtud, templanza, paciencia, piedad. ¿Cuál de esos atributos sentís que más necesitás desarrollar ahora? ¿En qué situación concreta de tu vida actual aparece esa necesidad?",
          "¿Cuál es la diferencia entre «cumplir mandamientos» como meta y «llegar a ser como Cristo» como meta? ¿Cómo cambia esa diferencia la manera en que vivís el Evangelio día a día?",
          "Valentina siente que siempre falla en lo mismo — no avanza en el mismo defecto de carácter año tras año. Dice: «Ya no sé si puedo cambiar.» Según el presidente Nelson y Éter 12:27, ¿qué le diría esta lección?",
          "3 Nefi 27:27 establece el estándar («así como yo soy»), Moroni 7:48 describe el mecanismo (orar para ser llenado), y DyC 4:5–6 lista los atributos específicos. ¿Cuál es el principio que une los tres pasajes sobre cómo se llega a ser semejante a Cristo?",
        ],
      },
    ],
  },
  {
    id: "r250-l27",
    title: "Lección 27: Hallar esperanza en la Luz y la Vida del mundo",
    type: "Religión 250",
    description: "",
    unitTitle: "Llegar a ser discípulos de Jesucristo",
    unitNumber: 6,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-27-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "Vivimos en el período histórico con más información disponible que cualquier generación anterior, y al mismo tiempo en uno de los momentos de mayor confusión moral, existencial y espiritual. La ansiedad entre jóvenes universitarios alcanza niveles sin precedentes. La desorientación sobre el propósito de la vida, sobre lo que es verdad, sobre en quién o en qué confiar, es un ruido de fondo constante. En ese contexto, la afirmación de Jesucristo en Juan 8:12 no es una frase devocional antigua — es una declaración sobre la realidad: «Yo soy la luz del mundo; el que me sigue, no andará en tinieblas, sino que tendrá la luz de la vida».\n\nDyC 88:11–13 expande esa imagen de manera que va mucho más allá de la metáfora espiritual: «La luz que ilumina el sol fue hecha por Cristo; y también la luz que ilumina la luna, es por Él; y la luz que ilumina las estrellas, e ilumina a la tierra... Y la luz que ilumina el sol es el mismo Cristo que ilumina vuestros entendimientos». La luz de Cristo no es solo una figura retórica — es la realidad cósmica que sostiene toda percepción, toda inteligencia, todo lo que existe. Eso significa que cuando buscás verdad — en cualquier campo, con sinceridad genuina — estás siguiendo la misma luz que el Salvador declaró ser.\n\nLa esperanza que Él ofrece no es optimismo ingenuo ni promesa de que todo saldrá bien en los términos que esperamos. Es certeza fundamentada en hechos ya ocurridos: la Expiación fue completada, la Resurrección ya sucedió. Esas realidades son faros fijos en un mar que puede ponerse turbulento. El título «El Cristo Viviente» lo resume con exactitud: «Él es la luz, la vida y la esperanza del mundo. Su camino es el sendero que lleva a la felicidad en esta vida y a la vida eterna en el mundo venidero.»",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "Juan 8:12",
            texto:
              "Otra vez Jesús les habló, diciendo: Yo soy la luz del mundo; el que me sigue, no andará en tinieblas, sino que tendrá la luz de la vida.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/john/8?lang=spa&id=p12#p12",
          },
          {
            referencia: "Doctrina y Convenios 88:11–13",
            texto:
              "Y la luz que ilumina el sol fue hecha por Cristo; y también la luz que ilumina la luna, es por Él; y la luz que ilumina las estrellas, e ilumina a la tierra, e ilumina vuestros entendimientos, que procede de la presencia de Dios para llenar el inmenso espacio; la luz que está en todas las cosas, que da vida a todas las cosas, que es la ley por la que todas las cosas son regidas — aun el poder de Dios que se sienta en el trono de los cielos — y el mismo Cristo es la luz y la vida del mundo.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/88?lang=spa&id=p11-p13#p11",
          },
          {
            referencia: "Moroni 10:32",
            texto:
              "Sí, venid a Cristo, y perfeccionaos en él, y absteneos de toda impiedad; y si os abstenéis de toda impiedad, y amáis a Dios con toda vuestra alma, mente y fuerza, entonces Su gracia os es suficiente.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/bofm/moro/10?lang=spa&id=p32#p32",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "«El Cristo Viviente: El Testimonio de los Apóstoles»",
        fuente:
          "Primera Presidencia y el Cuórum de los Doce Apóstoles, 1 de enero de 2000.",
        texto:
          "Él es el gran Rey Emanuel, que hoy está a la diestra de Su Padre. Él es la luz, la vida y la esperanza del mundo. Su camino es el sendero que lleva a la felicidad en esta vida y a la vida eterna en el mundo venidero.",
        link: "https://www.churchofjesuschrist.org/study/scriptures/the-living-christ-the-testimony-of-the-apostles?lang=spa&id=p13-13#p13",
      },
      {
        tipo: "enseñanza",
        autor: "Hermana Sharon Eubank",
        fuente:
          "«Cristo: la luz que resplandece en las tinieblas», Liahona, mayo de 2019, pág. 73.",
        texto:
          "Él es la fuente de nuestro poder, la Luz y la Vida del mundo. Sin una fuerte conexión a Él, comenzamos a morir espiritualmente... Avancen unos pasos más en la senda de los convenios, aun cuando esté demasiado oscuro para ver muy lejos; las luces volverán a encenderse.",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2019/04/42eubank?lang=spa&id=p3-3#p3",
        // La cita también incluye una segunda oración real del párrafo 30 (cierre del discurso); el ancla apunta solo al primer fragmento (p3) porque el rango p3-30 abarcaría prácticamente todo el discurso.
      },
      {
        tipo: "conclusion",
        contenido:
          "La última gran enseñanza de este curso sobre Cristo como Luz del mundo conecta con la primera: «Venid a mí todos los que estáis trabajados y cargados». Comenzó con una invitación personal a un Salvador personal, y termina igual. La esperanza que ofrece no es la solución de todos los problemas — es la presencia de Alguien que ya venció todos los problemas definitivos. Seguir a Cristo no garantiza que la vida no tenga oscuridad; garantiza que no andaremos en ella solos. La luz que ilumina el sol es el mismo Cristo que ilumina tu entendimiento. Eso no es poesía — es doctrina.",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "¿En qué área de tu vida sentís más oscuridad o confusión ahora mismo — relacional, académica, espiritual, existencial? Según Juan 8:12, ¿qué implicación concreta tiene la promesa «tendrá la luz de la vida» para esa área?",
          "DyC 88:11–13 dice que la luz que ilumina el sol es el mismo Cristo que ilumina los entendimientos. ¿Cómo cambia tu relación con el estudio y el conocimiento si los entendés como participación en la luz que Cristo sostiene?",
          "La hermana Eubank invitó a «avanzar unos pasos más en la senda de los convenios, aun cuando esté demasiado oscuro para ver muy lejos». ¿Cuándo experimentaste eso — no la solución completa sino la fe suficiente para dar el paso inmediato? ¿Cómo llegó?",
          "Moroni 10:32 dice «venid a Cristo y perfeccionaos en él». «El Cristo Viviente» dice «Él es la luz, la vida y la esperanza». ¿En qué se diferencia la esperanza basada en Cristo de otras formas de esperanza o positivismo que el mundo ofrece?",
          "¿Cómo podés ser esta semana un «reflector» de la luz de Cristo hacia alguien que estás viendo andar en oscuridad — no necesariamente con palabras religiosas, sino con presencia, atención y acción concreta?",
          "Juan 8:12 dice «el que me sigue tendrá la luz de la vida». DyC 88:11–13 dice que Cristo «ilumina vuestros entendimientos». Moroni 10:32 dice que Su gracia es suficiente. ¿Cuál es el principio doctrinal que une los tres pasajes sobre la relación entre Cristo y nuestra capacidad de ver con claridad?",
        ],
      },
    ],
  },
  {
    id: "r250-l28",
    title: "Lección 28: Ser testigos de Jesucristo",
    type: "Religión 250",
    description: "",
    unitTitle: "Llegar a ser discípulos de Jesucristo",
    unitNumber: 6,
    chapterUrl:
      "https://www.churchofjesuschrist.org/study/manual/jesus-christ-and-his-everlasting-gospel-class-prep-material-2023/lesson-28-class-prep?lang=spa",
    questions: [],
    secciones: [
      {
        tipo: "contexto",
        contenido:
          "La última reunión del Salvador con Sus discípulos antes de Su ascensión al cielo fue, en cierto sentido, una comisión. Hechos 1:8 registra Sus palabras: «Recibiréis poder cuando haya venido sobre vosotros el Espíritu Santo, y me seréis testigos en Jerusalén, en toda Judea, en Samaria, y hasta lo último de la tierra.» Eso no era solo para los Doce. Es para cada persona que ha tomado el nombre de Cristo sobre sí mediante el bautismo y ha prometido, en las palabras de Mosíah 18:9, «pararse como testigo de Dios en todo tiempo y en todas cosas y en todos los lugares en que estuvieres, hasta la muerte».\n\nEl curso de Religión 250 comenzó con la invitación del Salvador: «Venid a mí y aprended de mí». Las 28 lecciones que siguieron construyeron un retrato del Salvador a través de toda Su existencia — preterrenal, terrenal, posterrenal. Ahora la invitación final no es recibir más información sino responder: ¿quién vas a ser vos a partir de lo que aprendiste?\n\nEl élder Christofferson lo describió con precisión: «Las demás personas deberían poder ver en nosotros algo de Jesucristo. La forma en que actuemos, hablemos, el aspecto que tengamos y hasta nuestros pensamientos serán un reflejo de Él y de Su manera de proceder.» Ser testigo no requiere condiciones perfectas ni elocuencia. Requiere autenticidad: la persona que vio algo real en su experiencia del Salvador y no puede no decirlo. María Magdalena no tenía ninguna credencial especial cuando corrió a decirle a los discípulos «He visto al Señor». Solo tenía la experiencia, y eso fue suficiente.",
      },
      {
        tipo: "escrituras",
        citas: [
          {
            referencia: "Hechos 1:8",
            texto:
              "Pero recibiréis poder, cuando haya venido sobre vosotros el Espíritu Santo, y me seréis testigos en Jerusalén, en toda Judea, en Samaria, y hasta lo último de la tierra.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/acts/1?lang=spa&id=p8#p8",
          },
          {
            referencia: "Mosíah 18:9",
            texto:
              "Sí, y a pararte como testigo de Dios en todo tiempo y en todas cosas, y en todos los lugares en que estuvieres, hasta la muerte, para que seas redimido de Dios y seas contado con los de la primera resurrección.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/bofm/mosiah/18?lang=spa&id=p9#p9",
          },
          {
            referencia: "Juan 20:18",
            texto:
              "Fue entonces María Magdalena para dar a los discípulos las nuevas de que había visto al Señor, y que él le había dicho estas cosas.",
            link: "https://www.churchofjesuschrist.org/study/scriptures/nt/john/20?lang=spa&id=p18#p18",
          },
        ],
      },
      {
        tipo: "enseñanza",
        autor: "Élder D. Todd Christofferson",
        fuente:
          "«Cómo llegar a ser testigo de Cristo», Liahona, marzo de 2008, pág. 58.",
        texto:
          "Las demás personas deberían poder ver en nosotros algo de Jesucristo. La forma en que actuemos, hablemos, el aspecto que tengamos y hasta nuestros pensamientos serán un reflejo de Él y de Su manera de proceder... Y al imitar ese modelo, damos testimonio de Él.",
        link: "https://www.churchofjesuschrist.org/study/liahona/2008/03/becoming-a-witness-of-christ?lang=spa&id=p10-10#p10",
      },
      {
        tipo: "enseñanza",
        autor: "Élder Neil L. Andersen",
        fuente:
          "«Hablamos de Cristo», Liahona, noviembre de 2020, pág. 88.",
        texto:
          "A medida que el mundo hable menos de Jesucristo, hablemos nosotros más de Él. Conforme se manifieste nuestro verdadero carácter como discípulos Suyos, muchas personas a nuestro alrededor estarán preparadas para escuchar. Al compartir la luz que hemos recibido de Él, Su luz y Su trascendente poder salvador alumbrará a los que estén dispuestos a abrir el corazón.",
        link: "https://www.churchofjesuschrist.org/study/general-conference/2020/10/45andersen?lang=spa&id=p23-23#p23",
      },
      {
        tipo: "conclusion",
        contenido:
          "Esta lección, y este curso, termina donde comenzó la Iglesia primitiva: con testigos de primera mano que no podían guardar silencio sobre lo que habían visto. No tienes que haber visto ángeles ni haber tenido visiones. Tienes que haber experimentado algo real del Salvador — un cambio de dirección, una carga aliviada, un momento de claridad, una promesa cumplida. Esa experiencia, compartida con autenticidad en el «Jerusalén» de tu propia vida — tu familia, tus compañeros de facultad, tus amigos — es testimonio. El poder prometido en Hechos 1:8 está disponible. La pregunta es si estás dispuesto a usarlo.",
      },
      {
        tipo: "cuestionario",
        preguntas: [
          "Al terminar este curso, ¿cómo ha cambiado o profundizado tu testimonio de Jesucristo respecto a cuando empezaste? ¿Qué lección o enseñanza te marcó más?",
          "Mosíah 18:9 dice que el convenio de testigo es «en todo tiempo, en todas cosas, en todos los lugares». ¿En qué contexto específico de tu vida — facultad, trabajo, familia, amigos — te resulta más difícil ser testigo de Cristo? ¿Por qué?",
          "María Magdalena no tenía credencial ni elocuencia especial — solo tenía la experiencia de haber visto al Señor resucitado y fue a decirlo. ¿Qué experiencia personal tuya del Salvador podrías compartir con alguien esta semana con esa misma autenticidad?",
          "El élder Christofferson dice que «la forma en que actuemos, hablemos... y hasta nuestros pensamientos» reflejan a Cristo. ¿Qué atributo de Cristo — de los que estudiaste en este curso — querés priorizar cultivar en los próximos meses como forma de testimonio no verbal?",
          "El élder Andersen dice: «A medida que el mundo hable menos de Cristo, hablemos nosotros más de Él». ¿Cómo equilibrás ser un testigo auténtico con respetar la agencia de los demás y no volerte predicador invasivo?",
          "Hechos 1:8 dice que el poder de testificar viene del Espíritu Santo. Mosíah 18:9 describe la extensión del testimonio. Juan 20:18 muestra a María como primer testigo de la Resurrección. ¿Cuál es el principio que conecta los tres y que define qué hace posible y qué forma tiene el testimonio genuino?",
        ],
      },
    ],
  },
]