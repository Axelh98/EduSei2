...
function ProgressRing({ percentage, done, total }: { percentage: number; done: number; total: number }) {
  const r = 54;
  const circ = 2 * Math.PI * r;
  const offset = circ - (percentage / 100) * circ;
  const ringColor = percentage === 100 ? "#22c55e" : percentage >= 60 ? "#378ADD" : "#BA7517";

  return (
    <div className="relative flex items-center justify-center">
      <svg width="140" height="140" viewBox="0 0 140 140" className="-rotate-90">
        <circle cx="70" cy="70" r={r} fill="none" stroke="currentColor" strokeWidth="10" className="text-muted/60" />
        <circle
      ...
      <div className="absolute flex flex-col items-center justify-center text-center">
        <span className="text-3xl font-bold text-foreground leading-none">
          {percentage}<span className="text-lg font-medium text-muted-foreground">%</span>
        </span>
        <span className="mt-1 text-xs text-muted-foreground">{done}/{total}</span>
      </div>
    </div>
  );
}
...
function PendingLessonCard({ item, rawData }: { item: any; rawData: string }) {
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60 mb-0.5">
            ...
            {item.isFlat && (item.lesson as any).unitTitle && (
              <span className="ml-1 normal-case font-normal">· {(item.lesson as any).unitTitle}</span>
            )}
          </p>
          <h3 className="text-sm font-bold leading-snug text-foreground">{item.lesson.title}</h3>
        </div>
        <Link
...
export default function RecoveryPage({ searchParams }: PageProps) {
  ...
  const leccionesPending = pendingLessons.filter(
    (item) => !completados.includes(`${item?.categoryId}-${item?.lesson.title}`)
  );
  const hechas = leccionesDone.length;
  const porcentaje = totalLecciones > 0 ? Math.round((hechas / totalLecciones) * 100) : 0;
  const allDone = hechas === totalLecciones && totalLecciones > 0;

  const handleReportProgress = () => {
    const snapshot = { pendingLessons, completados, porcentaje };
    shareToWhatsApp((nombre) =>
      generateWhatsAppReport(snapshot.pendingLessons, snapshot.completados, snapshot.porcentaje, nombre)
    );
  };
        ...
        <main className="flex-1 px-4 py-8 md:px-6">
          <div className="mx-auto max-w-2xl">
            ...
            <header className="mb-8">
              ...
              <h1 className="font-serif text-2xl font-bold text-foreground">Tu camino al día</h1>
            </header>

            {/* Dashboard */}
            <div className="mb-8 rounded-2xl border border-border bg-card overflow-hidden">
              <div className="flex items-center gap-6 p-6">
                <ProgressRing percentage={porcentaje} done={hechas} total={totalLecciones} />
                <div className="flex-1 flex flex-col gap-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl bg-muted/50 px-3 py-2.5">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">Pendientes</p>
                      <p className="mt-0.5 text-2xl font-bold text-foreground">{leccionesPending.length}</p>
                    </div>
                    <div className="rounded-xl bg-green-500/8 px-3 py-2.5">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-green-700/60">Completadas</p>
                      <p className="mt-0.5 text-2xl font-bold text-green-600">{hechas}</p>
                    </div>
                  </div>
                  <button
              ...
              <section className="mb-6">
                ...
                <div className="flex flex-col gap-3">
                  {leccionesPending.map((item, index) =>
                    item ? <PendingLessonCard key={index} item={item} rawData={rawData} /> : null
                  )}
                </div>
              </section>
            )}
            ...
            {leccionesDone.length > 0 && (
              <section>
                ...
                >
                  ...
                  Completadas
                  <ChevronDown className={cn("ml-auto h-3.5 w-3.5 transition-transform duration-200", showCompleted && "rotate-180")} />
                </button>

                {showCompleted && (
                  <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-150">
                    {leccionesDone.map((item, index) =>
                      item ? <CompletedLessonCard key={index} item={item} /> : null
                    )}
                  </div>
                )}
              </section>
