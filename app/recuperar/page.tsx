            ...
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{leccionesHechas} de {totalLecciones} lecciones</p>
                </div>
              </div>
          ...
          <div className="grid gap-4">
            {pendingLessons.map((item, index) => {
              ...
              return (
                ...
                >
                  <div className={`flex items-center justify-between rounded-xl border p-5 transition-all ${
                    isDone 
                      ? "border-success/30 bg-success/5" 
                      : "border-border bg-card hover:-translate-y-1 hover:border-primary/50 hover:shadow-md"
                  }`}>
                    <div className="flex items-center gap-4">
                      <div className={`rounded-full p-2 transition-colors ${
                        isDone ? "bg-success text-white shadow-lg shadow-success/20" : "bg-primary/10 text-primary"
                      }`}>
                        {isDone ? <CheckCircle2 className="h-5 w-5" /> : <BookOpen className="h-5 w-5" />}
                      </div>
                      <div>
                        <p className={`text-[10px] font-bold uppercase tracking-widest ${isDone ? "text-success/60" : "text-muted-foreground"}`}>
                          {item?.category.name}
                        </p>
                        <h3 className={`font-bold transition-all ${
                          isDone ? "text-success/80 line-through decoration-2" : "text-foreground group-hover:text-primary"
                        }`}>
                          {item?.lesson.title}
                        </h3>
                      </div>
                    </div>
                    {!isDone && <ArrowRight className="h-5 w-5 text-muted-foreground group-hover:translate-x-1 transition-all" />}
                    {isDone && <span className="rounded-full bg-success/20 px-3 py-1 text-[10px] font-black text-success">LISTO</span>}
                  </div>
                </Link>
              )
            })}
